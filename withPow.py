...
def close_notepad_and_save(filename):
    ...
    if filename in result.stdout:
        ...
        # Use the full path to PowerShell to send Ctrl+S (save) and Alt+F4 (close) keystrokes to Notepad
        subprocess.run(
            r'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -command "$wshell = New-Object -ComObject wscript.shell; '
